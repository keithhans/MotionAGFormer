...
def calculate_left_arm_angles(hip, left_shoulder, right_shoulder, left_elbow):
    ...
    body_forward = np.cross(right_shoulder - left_shoulder, body_up)  # 身体前向的向量
    body_right = np.cross(body_up, body_forward)  # 身体右向的向量
...
if __name__ == "__main__":
    ...
    right_elbow = np.array([0.25813976, -0.02717768, 0.02717768])
    ...
    '''    
    左臂 Pitch (前后摆动): 66.54度
    左臂 Roll (左右摆动): -77.43度
    右臂 Pitch (前后摆动): -176.84度
    右臂 Roll (左右摆动): 168.36度
    '''